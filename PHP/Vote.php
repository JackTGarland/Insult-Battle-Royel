...
function vote(){
    ...
    try {
        ...
        $statment = $conn->prepare('UPDATE login SET voteRemaining = voteRemaining  - 1 WHERE username = :user AND voteRemaining > 0'); // Run's query on database. NOTE: unsure but on current understannding this is not sql injection vunruable in current state.
